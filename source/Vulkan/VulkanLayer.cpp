...
struct TLASResource : Resource {
    ...
    virtual ~TLASResource() {
        for (int i = 0; i < BLAS.size(); i++) {
            vkDestroyAccelerationStructureKHR(_ctx.device, BLAS[i].accel, _ctx.allocator);
            BLAS[i].buffer = {};
        }
        BLAS.clear();
        vkDestroyAccelerationStructureKHR(_ctx.device, TLAS.accel, _ctx.allocator);
        TLAS.buffer = {};
    }
};
...
TLAS CreateTLAS(const std::vector<BLASDesc>& blas, const std::string& name) {
    TLAS tlas;
    ...
    blasInputs.reserve(blas.size());

    for (const BLASDesc& desc : blas) {
    ...
    for (uint32_t idx = 0; idx < nbBlas; idx++) {
        ...
        _ctx.vkGetAccelerationStructureBuildSizesKHR(device, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
        ...
        if (batchSize >= batchLimit || idx == nbBlas - 1) {
            ...
            VkCommandBuffer cmdBuf = vkw::ctx().GetCurrentCommandBuffer();
            {
                for(const auto& idx : indices)
                {
                    ...
                    _ctx.vkCreateAccelerationStructureKHR(device, &createInfo, nullptr, &buildAs[idx].as.accel);
                    ...
                    _ctx.vkCmdBuildAccelerationStructuresKHR(cmdBuf, 1, &buildAs[idx].buildInfo, &buildAs[idx].rangeInfo);
    ...
    // ctx.BLAS.clear();
    for(auto& b : buildAs) {
        tlas.resource->BLAS.emplace_back(b.as);
    }
    scratchBuffer = {};
