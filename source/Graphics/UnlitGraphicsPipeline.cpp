...
void UnlitGraphicsPipeline::Setup() {
    ...
    desc.shaderStages[0].shaderBytes = FileManager::ReadRawBytes("bin/shader.vert.spv");
    desc.shaderStages[0].stageBit = VK_SHADER_STAGE_VERTEX_BIT;
    desc.shaderStages[1].shaderBytes = FileManager::ReadRawBytes("bin/shader.frag.spv");
